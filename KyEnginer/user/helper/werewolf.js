...
// alias biar gak perlu ubah logic lama
const toMs = ms
...
async function pagi(sock, x, data) {
    ...
    return await sock.sendMessage(x.room, {
        text: pagii(x),
        contextInfo: {
            externalAdReply: {
        ...
        for (let i = 0; i < x.player.length; i++) {
            if (x.player[i].role === "werewolf") {
                if (x.player[i].isdead != true) {
                    ...
                    await sock.sendMessage(x.player[i].id, {
                        text: textnya,
                        mentions: membernya,
                    });
                }
...
async function win(x, t, sock, data) {
    ...
    if (getWinner(x.room, data).status === false || x.iswin === false) {
        ...
        return await sock.sendMessage(sesinya, {
            text: textnya,
            contextInfo: {
                externalAdReply: {
                    title: "W E R E W O L F",
...
async function run(sock, id, data) {
    while (getWinner(id, data).status === null) {
        ...
        if (getWinner(id, data).status != null) {
            ...
            await pagi(sock, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await voting(sock, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
        ...
        if (getWinner(id, data).status != null) {
            ...
            await malam(sock, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
        } else {
            await skill(sock, sesi(id, data), data);
        }
        if (getWinner(id, data).status != null) break;
    }
    await win(sesi(id, data), 1, sock, data);
}

async function run_vote(sock, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
...
async function run_malam(sock, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
...
async function run_pagi(sock, id, data) {
    while (getWinner(id, data).status === null) {
        if (getWinner(id, data).status != null) {
            win(getWinner(id, data), 1, sock, data);
            break;
